...
## 2026-06-20 — Phase 6: Stage1 カスタムCNN本番化 ＋ 自前季節フィルタ

### 発端: 精度判定で偽陽性70%を実証
GT105 でバックログ(5/18-24 の実録音6269件)を素 6K で処理 → confirmed 434件の**約70%が偽陽性**。
福岡市西区北原・5月下旬(繁殖期)にありえない種が高確信度で出た:
- 冬鳥FP: Common Redpoll 177 / Rook 40 / Arctic Loon 38 / Red Crossbill 27 / Tundra Swan 20 …
- 別地域FP: Ryukyu Scops-Owl / Whistling Green-Pigeon(南西諸島種)

**根本原因(コードで確認)**: ①素6K＋通年eBirdホワイトリスト(296種)で季節無視。`analyzer.custom_species_list`
をセットすると birdnetlib の週次予測(`set_predicted_species_list_from_position`)が上書き・無効化。
②さらに**カスタム分類器使用時は birdnetlib の eBird 季節/地域予測が構造的に完全停止**
(`analyzer.py`: `if recording.lon and recording.lat and self.classifier_model_path == None`)。
→ カスタムCNNでは BirdNET内蔵フィルタが使えない＝**自前の季節フィルタが必須**。

### 実装
- **Stage1 = 九州北部 applicable CNN(165クラス)** へ切替。`settings.birdnet_model_path/labels_path`。
- **dispatch を sci 駆動化** (`stage2_refine.load_dispatch_map` / `process.py`): 検出の `scientific_name`
  でgroup解決。素6K↔カスタムCNN の英名表記揺れ("Gray"↔"Grey", "Oriental"↔"Eurasian" Magpie 等)に非依存。
- **自前季節フィルタ**(`tools/build_seasonal_occurrence.py` → `data/seasonal_occurrence.csv`):
  birdnetlib 同梱 eBird メタモデル(`species_class.return_list`)を録音地点で week 1-48 ぶん叩き、
  165ラベルへ**学名で突合**して 週×種×eBirdスコア表を生成。ランタイム(process.py)は録音の週の
  在期種(score≥0.15)だけを `custom_species_list` にセット(lat/lon は渡さない=ValueError回避)。
- **カスタム上書き層** `data/seasonal_overrides.csv`: eBirdメタは多くの冬鳥で夏床を過大評価
  (Rook 夏0.31, Merganser 夏0.30)し閾値で落ちない → 明確な冬鳥を week18-36 で remove。
  現状20種シード(冬カモ/冬の地上ツグミ・アトリ・ホオジロ類/ミヤマガラス)。★公的データ(eBird bar-chart/
  環境省モニタリングサイト1000)で要裏取り。これが「リストを自作・拡張」する成長面。

### 検証(実フィールド A/B・素比較ガード)
救出録音52件(FP系31/TP系21)を一時DBで素6K vs カスタムCNN+季節フィルタ比較:
- **冬鳥/別地域FP: 36件 → 0件**。留鳥(カラス2種/スズメ/アオサギ)は非劣化で残存。
- 残課題: ①Lesser Cuckoo(ホトトギス)欠落=165クラスに無い(現地で夜鳴き確認済の実在種)。
  ②難録音をゴイサギに寄せる癖(テストが難例偏重ゆえの可能性、要監視)。
- ガード合格 → **本番 settings 切替**(custom CNN + seasonal enabled)。ロールバックは model=null で即復帰。

### 再学習候補リスト立ち上げ → `docs/retrain_candidates.md`
165クラス欠落のうち当地で実在しうる種を抽出。Tier1=ホトトギス+dispatch欠落3種(スズガモ/オオセグロ/
ズグロカモメ)、Tier2=カッコウ類/コチドリ/アオバト/コノハズク等。次回 applicable モデル再学習で追加。

### 設計メモ(将来): 検出粒度
現状は「種×ファイル(60s)で1レコード(最良窓+det_count)」。本質的には
「音声セグメント(イベント=連続チャンク)→その鳥(複数種可)」が正しい粒度(学習データ=ラベル付き
セグメントそのもの, レビューも直感的)。go-live前のコールドスタートの今が作り直しの分岐点。→ 別途検討。
